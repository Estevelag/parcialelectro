...
ei[0,0:180]=sc.epsilon_0
ei[0,180:200]=sc.epsilon_0*2.178576
ei[0,200:700]=sc.epsilon_0*12
ei[0,700:720]=sc.epsilon_0*2.178576
ei[0,720:1001]=sc.epsilon_0
ui=np.zeros((1,Npts),dtype=float)
ui[0,0:170]=sc.mu_0
ui[0,170:200]=sc.mu_0
ui[0,200:700]=sc.mu_0
ui[0,700:770]=sc.mu_0
ui[0,770:1001]=sc.mu_0
...
line1, = ax.plot(x, y, 'r-')
ax.axvline(x=200*dx)
ax.axvline(x=180*dx,color='r')
ax.axvline(x=720*dx,color='r')
ax.axvline(x=700*dx)
...
for i in range(0, 4000):
    ...
    if (i<200) and (f*dt*i<1):
        ...
        Ez[0,pc]=E0*np.sin(2*np.pi*f*dt*i)*exp(-i*1000)
    Ez[0,0]=b
    Ez[0,Npts-1]=a
    line1.set_ydata(Ez[0,:])
    fig.canvas.draw()#####actualizacion de la grafica
    fig.canvas.flush_events()
print("reflectancia y transmitancia =",-Ez[0,89],Ez[0,249])